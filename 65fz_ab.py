            ...
            if key in ('u', 'U'):
                ...
                try:
                    ...
                    if dir_key not in ('u', 'd'):
                        ...
                        print(f"[OK] 已加入队列: 卖出{sell_dir} 90%")
                except Exception as e:
                    print(f"[警告] 输入错误: {e}")
                finally:
                    input_mode = False
            ...
            if manual_order_queue:
                ...
                if action == "BUY":
                    ...
                    if entry_dir == m_dir and entry_size > 0:
                        sell_size   = round(entry_size * 0.9, 2)
                        sell_order_by_size(m_token, sell_size, m_price, f"卖出{m_dir} 90%")
                        entry_size  = round(entry_size - sell_size, 2)
                        total_spent = round(total_spent * 0.1, 2)
                    else:
                    ...
                    if entry_size > 0 and entry_dir:
                        ...
                        sell_size   = round(entry_size * 0.9, 2)
                        sell_order_by_size(m_token, sell_size, m_price, f"卖出{entry_dir} 90%")
                        entry_size  = round(entry_size - sell_size, 2)
                        total_spent = round(total_spent * 0.1, 2)
                    else:
                        print("[警告] 无持仓记录")
